...
data <- list.files(path = "MergingFolder",     # Identify all csv files in folder
...
ParsiDF$Extreme1 <- is_extreme(ParsiDF$MaxInterpenetration)

ParsiDF$Extreme2 <- is_extreme(ParsiDF$AverageInterpenetration)
#ParsiDF$Extreme1[ParsiDF$Extreme1 == TRUE]  <- NA
#ParsiDF$Extreme2[ParsiDF$Extreme2 == TRUE]  <- NA
...
Let's Check the intesities of the electrotactile feedback
...
OK, for every DV (i.e., Sensatio, Pain , and ActualValue)  we have the same results 
...
I interpret them as follows: During the 1st part (i.e., until the middle calibration ) the individuals get familiarized with the electrotactile feedback. Then, the variation drops significantly! So, the most reliable calibration appears the middle one (considering that the final doesnt differ, so it seems redundant). Importantly, these results utterly support the familiarization hypothesis regarding the results of the ANOVAs and 
Comparisons between the feedback types, as well as it explains why the electrotactile and both have greater effect size on the 2nd part!!!




Let's check the effect size now.
...
#Check if there is a correlation between calibration and the scores
ParsiDF$Calibration <- intensities$Calibration
view(intensities)
...
We have Large (Very large in some) Effect in every comparison.
